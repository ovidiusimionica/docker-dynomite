...
# latest release; uncomment to use a fixed release or else we use master
# RUN git checkout v0.5.9-5_MuslCompatiblity
...
RUN mkdir -p /var/run/dynomitedb
RUN mkdir -p /etc/dynomitedb

COPY supervisord.conf /etc/supervisor/conf.d/supervisord.conf
COPY dynomite.yaml /etc/dynomitedb/dynomite.yaml
COPY dynomite.pem /etc/dynomitedb/dynomite.pem
# Set the entry-point command to supervisord
